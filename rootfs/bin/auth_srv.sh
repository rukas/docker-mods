...
auth_header="Authorization: Bearer ${HOMEASSISTANT_TOKEN}"
content_type="Content-Type: application/x-www-form-urlencoded"

if curl -s -f -X POST -d "${REQUEST_BODY}" -H "${content_type}" -H "${auth_header}" ${HOMEASSISTANT_URL}/auth > /dev/null; then
